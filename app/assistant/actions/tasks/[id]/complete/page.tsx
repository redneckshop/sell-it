...
export default function AssistantCompleteTaskPage() {
  ...
  useEffect(() => {
    async function loadTask() {
      ...
      const { data, error } = await supabase
        .from("tasks")
        .select(
          "id, title, description, due_date, priority, status, company_id, contact_id, opportunity_id, updated_at"
        )
        .eq("id", taskId)
        .single();
  ...
  async function handleConfirmComplete(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      setSuccessMessage("Task marked Completed. Completion metadata was saved too.");
    }
